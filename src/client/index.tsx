/**
 * Browser half of dsh-step-trace: the Step trace page inside Settings.
 *
...
function SteptracePage() {
  const { html, error, reload } = usePanel()
  if (error !== null) {
    return (
      <section>
        <p role="alert">Step trace panel failed to load: {error}</p>
        <button type="button" onClick={reload}>Retry</button>
      </section>
    )
  }
  if (html === null) return <p aria-live="polite">Loading the Step trace panel…</p>
